================================================================================
    MFC 라이브러리 : SWLogDebuggingTool(W) 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 SWLogDebuggingTool(W) 응용 프로그램을 
만들었습니다.  이 응용 프로그램은 MFC(Microsoft Foundation Classes)의
기본적인 사용법을 보여 주며 응용 프로그램 작성을 위한 출발점을 제공합니다.

이 파일에는 SWLogDebuggingTool(W) 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

SWLogDebuggingTool(W).vcproj
    응용 프로그램 마법사를 사용하여 생성된 VC++ 프로젝트의 주 프로젝트 파일입니다.
    파일을 생성한 Visual C++ 버전에 대한 정보와
    응용 프로그램 마법사를 사용하여 선택한 플랫폼, 구성 및 프로젝트
    기능에 대한 정보가 들어 있습니다.

SWLogDebuggingTool(W).h
    응용 프로그램의 기본 헤더 파일입니다.  여기에는 다른 프로젝트 관련
    헤더(Resource.h 포함)가 들어 있고 CSWLogDebuggingToolWApp 응용 프로그램
    클래스를 선언합니다.

SWLogDebuggingTool(W).cpp
    응용 프로그램 클래스 CSWLogDebuggingToolWApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.

SWLogDebuggingTool(W).rc
...
res\SWLogDebuggingTool(W).ico
    응용 프로그램의 아이콘으로 사용되는 아이콘 파일입니다.  이 아이콘은
    주 리소스 파일인 SWLogDebuggingTool(W).rc에 의해 포함됩니다.

res\SWLogDebuggingToolW.rc2
...
res\Toolbar.bmp
    ...
    SWLogDebuggingTool(W).rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음
    단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////
...
SWLogDebuggingTool(W)Doc.h, SWLogDebuggingTool(W)Doc.cpp - 문서
    이 파일에는 CSWLogDebuggingToolWDoc 클래스가 들어 있습니다.  이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CSWLogDebuggingToolWDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.

SWLogDebuggingTool(W)View.h, SWLogDebuggingTool(W)View.cpp - 문서 뷰
    이 파일에는 CSWLogDebuggingToolWView 클래스가 들어 있습니다.
    CSWLogDebuggingToolWView 개체는 CSWLogDebuggingToolWDoc 개체를 보는 데 사용됩니다.

res\SWLogDebuggingTool(W)Doc.ico
    CSWLogDebuggingToolWDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
    아이콘 파일입니다.  이 아이콘은 주 리소스 파일인 SWLogDebuggingTool(W).rc에
    의해 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일인 SWLogDebuggingTool(W).pch와
